...
/** Listagem de equipamentos: busca/filtro, tabela e acoes de editar/excluir. */
public class EquipamentosController {
    ...
    public void initialize() {
        ...
        colPreco.setCellValueFactory(c -> new SimpleStringProperty(FormatUtil.currency(c.getValue().getPreco())));
    ...
    private void aplicarFiltro() {
        String termo = searchField.getText() == null ? "" : searchField.getText().trim().toLowerCase();
        filteredList.setPredicate(eq -> {
            if (termo.isEmpty()) {
                return true;
            }
            if (filtroSerie.isSelected()) {
                return eq.getNumeroSerie() != null && eq.getNumeroSerie().toLowerCase().contains(termo);
            } else if (filtroResponsavel.isSelected()) {
                return eq.getResponsavel() != null && eq.getResponsavel().getNome().toLowerCase().contains(termo);
            } else if (filtroLocal.isSelected()) {
                return eq.getLocal() != null && eq.getLocal().getDescricao().toLowerCase().contains(termo);
            } else {
    ...
    private void atualizarSubtitulo() {
        ...
        subtitleLabel.setText(total + (total == 1 ? " equipamento cadastrado" : " equipamentos cadastrados"));
    }
    ...
    private void abrirModal(Equipamento existente) {
        EquipamentoFormController controller = AppContext.getMainController().showModal("equipamento_form.fxml");
